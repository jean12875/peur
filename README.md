...
Même installé en mode app, une bande noire en bas et les icônes système en haut ont été signalées sur iPhone. Cause probable : le calcul de hauteur `100dvh` se comporte mal dans certaines versions de WKWebView (le moteur qu'utilise une PWA "standalone" sur iOS), en laissant une bande vide au lieu de remplir tout l'écran. Correctif appliqué : la hauteur de la page est maintenant recalculée en JavaScript (`window.innerHeight`) et posée comme repli avant `100dvh`, avec re-calcul automatique à chaque rotation/redimensionnement ; les boutons et l'icône réglages tiennent aussi compte des zones encoche/barre d'accueil (`env(safe-area-inset-*)`). Ce correctif n'a pas pu être testé sur un vrai iPhone depuis cet environnement — à confirmer après une réinstallation (supprimer l'icône puis "Ajouter à l'écran d'accueil" à nouveau, pour forcer un chargement propre).
