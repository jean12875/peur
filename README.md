...
Même installé en mode app, une bande noire en bas a persisté sur iPhone après un premier correctif. Cause trouvée : la page utilisait `100dvh` en dernier recours après un calcul JS plus fiable — mais `100dvh` est reconnu comme valide par la WKWebView standalone d'iOS, même quand sa valeur calculée est fausse dans ce mode, donc il gagnait quand même la priorité CSS et annulait le correctif JS. `100dvh` a été retiré : seule la hauteur mesurée en JavaScript (`window.innerHeight`, recalculée au chargement, à la rotation, et au retour au premier plan) pilote maintenant la page. Les boutons et l'icône réglages tiennent aussi compte des zones encoche/barre d'accueil (`env(safe-area-inset-*)`). Comme précédemment, ce correctif n'a pas pu être testé sur un vrai iPhone depuis cet environnement.

La bande a persisté après ce premier correctif. Ajouté en plus : la hauteur utilise maintenant `visualViewport.height` en priorité (plus fiable que `innerHeight` sur iOS dans certains cas), et le calcul est relancé plusieurs fois dans les 2 secondes suivant le chargement (au cas où la vraie hauteur visible ne se stabilise qu'après un court délai en mode standalone). Point important si le correctif ne semble toujours pas s'appliquer : une PWA installée sur iOS peut garder un ancien processus en mémoire même quand on la "réouvre" depuis l'icône — il faut la fermer complètement depuis le multitâche (glisser vers le haut et maintenir, puis balayer LE TEST hors de l'écran) avant de la relancer, sinon elle peut afficher une version mise en cache plus ancienne.
